...
3. Escolher o mapeamento:
   ...
   - Nenhum casa → escrever `mapeamentos/<corretora>-<documento>.yaml` no workspace, com `verificado-contra-export-real: false`, `numeros:` lido da amostra (`pt-BR` se o documento escreve `1.234,56`; `en-US` se escreve `1,234.56` — é obrigatório e o motor não adivinha), regras explícitas para cada tipo de linha visto na amostra, e a conciliação que o documento permite (saldo corrente se há coluna de saldo; valor da linha se há qty × preço = valor; senão total declarado). **Mostrar o YAML ao usuário e esperar o de acordo.**
