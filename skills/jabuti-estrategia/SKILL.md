...
- **Quanto, não em quê.** Esta etapa decide o tamanho de cada bloco (mín, alvo, máx) e os caps de concentração. Qual ativo entra em cada bloco é do `/jabuti-micro`.
...
9. Com zero erros: no `SETUP.md`, marcar `- [x]` na linha do `/jabuti-estrategia` e acrescentar, indentada abaixo dela, uma linha `- [ ] `/jabuti-micro <bloco>`` para cada bloco com alvo maior que zero, na ordem da tabela. Trocar `Próximo:` para `` `/jabuti-importar` ``.
...
- **Só ETF, sem picking**: legítimo, e é caminho previsto. As bandas continuam iguais; a diferença aparece no `/jabuti-micro`.

## O que esta skill NÃO faz

- **Não escolhe ativo** (→ `/jabuti-micro <bloco>`)
