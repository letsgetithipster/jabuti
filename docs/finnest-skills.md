...
| `/jabuti-vazamentos` | quais cobranças recorrentes eu corto para aportar mais? | `get_data_freshness_status`, `list_insight_subscriptions` (ou `detect_subscriptions`), `get_subscription_stack_load`, `get_category_spending`, `list_top_merchants` | um log de decisão em `logs/decisoes/` |
